## 🔔 Garbage Collection
**[1. C/C++에서의 메모리 관리 (UnManaged Code)]**<br>
...
**객체 중 쓰레기인것과 쓰레기가 아닌 것을 완벽하게 분리해서 쓰레기들을 수거하는 가비지 컬렉터라는 것이 존재한다.**<br>
...
**여기서 만약 0세대 객체들의 용량이 0세대 가비지 컬렉션 임계치를 넘어서게되면,**<br>
...
-> **H I J K가 0세대에 할당됨.**<br>
하지만 이전 과정을 통해 **1세대의 임계치가 초과되었기 때문에 1세대에 대해 가비지 컬렉션을 수행한다.**<br>
<br>

![세대별6](https://user-images.githubusercontent.com/43705434/132116911-dba725a0-4b7a-40ac-b349-ab06619d19ea.PNG)<br>
<br>

이 때 **가비지 컬렉터는 하위 세대에 대해서도 가비지 컬렉션을 수행**하기 때문에 0세대와 1세대에 대한 가비지 컬렉션이 수행된다.<br>
...
👉 **이해를 위한 세대별 GC 설명 추가**<br>
![세대별 gc](https://user-images.githubusercontent.com/43705434/132119862-f6b3658b-0ed1-4c13-bd9e-dc970780aa31.PNG)<br>
<br>

01.   새로 할당된 객체들 모두 0세대에 포함된다.<br>
0세대가 임계치에 도달하게되면 가비지 컬렉터는 0세대에 대해 가비지 컬렉션을 수행하게 된다.<br>

02.   가비지 컬렉션 수행 후 살아남은 객체들을 1세대로 옮겨지게 된다.<br>

03.   이후 또다른 객체가 할당되어 들어온다면 그 객체들은 당연히 0세대에 포함된다.<br>
0세대가 또 다시 임계치에 도달하게 되면 가비지 컬렉터는 0세대에 대해 가비지 컬렉션을 수행한다.<br>

04.   이번에는 1세대가 임계치에 도달하게 되었다.<br>
마찬가지로 가비지 컬렉션을 수행한다.(1, 0 둘다 수행됨.) <br>

05.   1세대의 가비지 컬렉션에서 살아남은 객체는 2세대로 옮겨지게 된다.<br>
객체의 수명은 이런식으로 가비지 컬렉터에서 오래 살아남았다는 기준으로 정해지게 된다.<br>

06.   이러한 방식으로 0세대, 1세대, 2세대를 구분하여 각각의 임계치에 도달하면<br>
해당 세대에 대해 가비지 컬렉션을 수행하게 된다.<br>

-->  0세대가 포화된다면 오로지 0세대만 가비지 컬렉션을 수행 한 후 1세대로 이동<br>
1세대가 포화된다면 0세대와 1세대만 가비지 컬렉션을 수행 한 수 각각 1세대, 2세대로 이동<br>
-->  2세대가 포화되게 된다면 더 이상 다른 곳으로 옮겨지지 않기 때문에,<br>
전체 가비지 컬렉션을 수행하게 된다.<br>
<br>

위의 세대별 가비지 컬렉션 알고리즘으로 인해 ⭐⭐ **생성과 삭제가 빈번한 객체에 대해서만<br>
가비지 컬렉터가 신경을 쓸 수있게 하는 메커니즘이 완성되었다.** ⭐⭐<br>
<br>
<br>
...
객체를 할당하게 된다.** ⭐<br>
-> C-Runtime 처럼 **탐색 오버헤드**가 존재한다는 것.<br>
...
**Mark:** 필요한 Object를 표시 한다. (1bit 사용)<br>
...
⭐ **결론** ⭐<br>
**mark and sweep 알고리즘은 Root들을 추적하며 객체들이 가비지인지 아닌지 1bit로 표시한 후<br>
해당 객체들을 힙을 돌며 제거해주는 알고리즘이다. 하지만 메모리들을 재배치하지 않기 때문에<br>
메모리 단편화가 일어날 수 있다.**<br>
<br>
<br>
...
그렇기 때문에 우리는 Mark-Sweep-Compact 알고리즘을 사용한다.**<br>

**Mark-Sweep-Compact 알고리즘은 표시된 모든 개체를 메모리 영역의 시작 부분으로 끌어당겨 Mark 및 Sweep의 단점을 해결한다.<br>
이 접근 방식의 단점은 모든 개체를 새 위치에 복사하고 해당 개체에 대한 모든 참조를 업데이트해야 하므로 GC 일시 중지 기간이 증가한다는 것<br>
하지만 이러한 방식을 사용하면 메모리 단편화 문제가 발생하지 않기 때문에 새 개체 할당 비용이 매우 저렴해진다.**<br>
<br>
...
⭐ **결론** ⭐<br>
**mark sweep compact 알고리즘은 mark and sweep 알고리즘에 살아남은 객체들의 메모리를 재배치해 정리하는 과정이 추가된 것이다.<br>
그렇기 때문에 메모리 단편화 문제가 일어나지 않아 새 객체 할당 비용이 저렴해진다.<br>
하지만 재배치하는 과정이 추가된 만큼 GC로 인한 프로그램의 일시 정지 시간이 늘어나게 된다.**<br>
<br>
<br>
...
Mark-Sweep-Compact와 유사하게 메모리를 재배치하는 알고리즘 이다.<br>
**중요한 차이점은 재배치할 위치가 아예 다른 메모리 영역이라는 점이다.<br>
Mark and Copy 방식은 마킹과 동시에 재배치를 할 수 있다는 장점이 있다.<br>
단점은 살아남은 개체를 수용할 수 있을 만큼 충분히 큰 메모리 영역이 하나 더 필요하다는 것이다.**<br>

Mark and Copy는 위와 같이 GC를 하기전 두가지 메모리 공간을 사용한다.<br>
A는 메모리안에 사용된 공간과 사용되지 않는 공간이 존재한다. 하지만 B는 완전히 비어있다.<br>

**가비지 콜렉션이 실행 될때 가비지가 아닌 객체를 마킹함과 동시에 두번째 메모리 영역에 재배치시켜버린다.<br>
위의 단계는 Object가 Mark되는 것과 동시에 동작 할 수있고 이렇게 하면 프로그램이 GC에 의해 중단되는 시간을 줄여 준다.**<br>
<br>

⭐ **결론** ⭐<br>
**Mark and Copy 알고리즘은 메모리를 재배치한다는 점에서 Mark sweep compact 알고리즘과 유사한데,<br>
재배치할 영역이 완전히 새로운 메모리 영역이라는 점이 차이점이다.<br>
이러한 방식은 마킹과 재배치를 동시에 할 수 있어 GC로 인한 프로그램 일시 정지 시간을 줄일 수 있지만<br>
큰 메모리 영역을 하나 더 필요로 한다는 단점이 있다.**<br>
<br>
<br>
...
해당 리소스들의 자원을 해제하기 위해 Using - IDisposable ⭐을 사용하는 것이 좋다.**<br>
-> 소멸자와 같은 finalizer를 사용할 수도 있지만 해당 방법은 단점이 많기 때문에<br>
-> 이보단 IDisposable을 이용해 unmanaged 리소스가 빠르게 해제될 수 있도록 구현하는 것이 좋다.<br>
다음 [파일](https://github.com/tlagmltjq11/CSharp_and_OOP/blob/main/C%23/Dispose%20&%20Finalizer.md)을 참고해서 복기하도록 하자.<br>
<br>
<br>

## 🔔 추신
과거에 유니티는 재정 상 최신버전의 GC를 적용할 수 없어 구식 GC를 사용하고 있었다고 한다.<br>
...
https://everyday-devup.tistory.com/14 <br>
https://mia-dahae.tistory.com/105 (알고리즘) <br>
https://plumbr.io/handbook/garbage-collection-algorithms (알고리즘) <br>
